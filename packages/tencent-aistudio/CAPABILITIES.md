...
## 16. LIVE VERIFICATION LOG — 2026-09-19 (DOM + functionality, end-to-end)

Posture hard requirement (re-verified): **headed real Chrome under Xvfb only**
(`DISPLAY=:97 UI2API_HEADED=1 UI2API_CHROME=1`) — EdgeOne still serves HTTP 567
to headless Chromium. Session: injected snapshot lock 2026-09-18T14:42:47Z
(3 cookies: `hunyuan_token` + `hunyuan_user` + `hunyuan_source` on `.tencent.ai`;
host `aistudio.tencent.ai`).

| Capability | Result | Verified mechanism (DOM) |
|---|---|---|
| `tencent_aistudio_chat` | ✔ proof PASS (13717 + 12 + 72; "Reply with ONLY the number") | composer `textarea.t-textarea__inner` (placeholder "Ask me anything", TDesign) → typed input + Enter → streamed answer `.agent-chat__bubble--ai .hyc-content-md` → markdown body `.hyc-common-markdown`; human bubbles `.agent-chat__bubble--human .hyc-content-text`; completion marker "Completed"; footer "The Above Content is Generated by AI". URL after send: `/chat/HunyuanDefault?modelId=hy4-preview-g…` (model "Hy4 preview", mode "High"). |
| `tencent_aistudio_conversation_crud` | ✔ LIST + OPEN (2026-09-23 live probe; runner implemented) | History menu → **`/chat-history`** renders the user's REAL dated conversation list as `div.list-item` rows (title, model, type — e.g. "PrimeNumberCheck / Hy4 preview / Text Generation", "MorningSunlightPodcastScript / Hy4 preview / Text Generation"). Rows are DIVs (no `href` — conversation ids are click-only, revealed by the navigation URL). Clicking a row navigates into `/chat/HunyuanDefault/<id>?from=history&jumpType=model&modelId=hy4-preview-g` and the clicked conversation loads (live proof: clicked "PrimeNumberCheck" → the "Is 17 prime?" thread reopened). **rename/delete/clear: NO DOM control located on this revision → honest ok:false for those actions (wire endpoints in §1).** |
| conversation RENAME/DELETE/CLEAR | ✗ honest blocked | no rename/delete/clear control found in the live History surface (list rows are click-to-open only). |

## 17. LIVE VERIFICATION LOG — 2026-09-23 (GOAL 15 probe pass, headed real Chrome + snapshot)

Posture: Xvfb `:99` + `UI2API_HEADED=1 UI2API_CHROME=1`, injected 2026-09-18 snapshot lock. Four probe passes, one browser launch each.

| Capability | Result | Measured (live DOM / route) |
|---|---|---|
| `tencent_aistudio_chat` | ✔ ok:true (re-verified) | fresh round-trip "Reply with ONLY the number 415." → answer `415` (2026-09-23T14:49:06Z). Composer + answer selectors unchanged. |
| `tencent_aistudio_conversation_crud` | ✔ list + open (see §16 row) | `/chat-history` list read-back (17 list-item rows, dated groups incl. 2026-09-23); row click → `/chat/HunyuanDefault/dapuegc2c3mak6ovd5g0?from=history`. |
| `tencent_aistudio_web_search` | ✗ HONEST BLOCKED — measured | scanned every composer toolbar/control-extra element + all DOM matching 搜索/search/联网/deep search: **no search toggle exists** on the current Hy4 preview composer. `searchDeepMode` still wire-proven in bundles only. |
| `tencent_aistudio_deep_think` | ✗ HONEST BLOCKED — measured (output renders) | no deep-think **toggle** in composer (only the "High" model-mode chip). BUT the live answer DOM carried `Deep thinking completed（Ran for 1.823s）` (`.hy-detail-block-header-title`) — the deep-thought OUTPUT renders on Hy4 preview by default; there is simply no switch to expose. |
| `tencent_aistudio_image_gen` | ✗ HONEST BLOCKED — measured | no image-gen trigger in aistudio; `/image` → "Current Page Does Not Exist". "3D Studio" menu = SEPARATE app `hy3d.tencent.ai` (text-to-3D / image-to-3D) — different surface (DIT aistudio image gen is not reachable from chat). |
| `tencent_aistudio_code_run` | ✗ HONEST BLOCKED — measured | no coder/runCode trigger in composer or `/code` (route → "Current Page Does Not Exist"). |
| `tencent_aistudio_file_upload` | ✗ HONEST BLOCKED — measured | composer has a files zone `hy-chat-input__content-top-content-wrapper--no-files` but **no `input[type=file]` and no attach/upload button** anywhere in the live DOM. |
| `tencent_aistudio_tts` | ✗ HONEST BLOCKED — measured | no TTS/朗读/speaker button anywhere in live DOM; `/tts` → "Current Page Does Not Exist". |
| `tencent_aistudio_podcast` | ✗ HONEST BLOCKED — measured | no podcast/播客 trigger in composer or side menu; `/podcast` → "Current Page Does Not Exist". (History holds a prior "MorningSunlightPodcastScript" thread — the feature has existed, current trigger not located.) |
| `tencent_aistudio_translations` | ✗ HONEST BLOCKED — measured | no translation/翻译 control in composer; `/translate` → "Current Page Does Not Exist". |
