...
| conversation/history DOM | ✗ no anchors on this revision | the "History" entry (`div.layout-menu__menu-history` in the side menu) renders **no** `a[href*='/chat/']` list on the 2026-09-19 revision — conversation CRUD/list stays honest ok:false (wire endpoints in §1/§15, shapes for a future live History capture). |

Side menu inventory (live): Chat / 3D Studio / Get API Key / Docs / History / New.
Cold-boot: composer appears ~2s, but sends typed before ~5–8s are silently
dropped — `preComposeDelayMs: 8000` (jittered +0–600ms) is baked into the
profile, and runner DOM paths poll for their target nodes (up to 15s).

All other tencent capabilities (web_search/deep_think/image_gen/code_run/
file_upload/tts/podcast/translations) remain wire-mapped, DOM-unverified →
runner returns honest ok:false with mechanism notes until a live capture
confirms the composer/toolbar toggles. Never claim verified without a live
round-trip (AGENTS.md red line).
