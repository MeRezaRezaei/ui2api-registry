...
> STATUS (2026-09-20): **live-probed on the attached real Chrome 152 (CDP
> attach via `launchBrowser()`, UI2API_ATTACH_PORT=9222).**
> - `youtube_search` — **VERIFIED** (live round-trip: 11 results read).
> - `youtube_transcript` — **UI path VERIFIED to the network gate; segment
>   read-back BLOCKED by a login gate** (site's own `get_transcript` →
>   HTTP 400 "Precondition check failed" for anonymous sessions).
...
| id | flow | status |
|---|---|---|
| `youtube_search` | navigate `https://www.youtube.com/results?search_query={q}` (the site's own page), read rendered rows | **verified-2026-09-20** |
...
account-bound precondition). The UI selectors are correct and verified; a
**logged-in `session.lock.json` capture** is required before segment
read-back can be claimed verified. Until then the runner reports the gate
honestly (ok:false with the 400 explanation).
...
transcript**: capture a logged-in session
(`ui2api profile capture "https://www.youtube.com" --login`) and lock it in
`session.lock.json`. See `session.lock.json` (status `awaiting-capture`).

## Remaining checklist

1. Capture a LOGGED-IN YouTube session into the package.
2. Re-run `youtube_transcript`; confirm `ytd-transcript-segment-renderer`
   rows render and `.segment-text`/`.segment-timestamp` read back.
3. Record PASS proof and flip the transcript status to verified; update
